	                ...
	                ChargeFactory.recharge(url, $scope.charge.money).then(function(data) {
	                        ...
	                        if (data.success) {
	                            if (url == '/api/recharge_wxpay_gdiex') {
	                                 // 自定义弹窗
	                               $ionicPopup.show({
	                                   template: '<input type="text" class="code" disabled="disabled" value="验证码: '+data.object+'">' + '<div>复制验证码到OUKEYE服务中心公众号支付。</div>',
	                                   title: '充值信息',
	                                   subTitle: '长按验证码可复制',
	                                   buttons: [{
	                                       text: '<b>确定</b>',
